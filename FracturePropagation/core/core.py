...
def get_time_str():
    return datetime.datetime.strftime(get_curr_time(), "%Y-%m-%d-%H-%M-%S")


# input_choose = input("是否导入参数（y/n）")
# 手动输入参数
def input_choose_n(data_json, filename='test', input_choose="n"):
    # name = input("算例文件夹备注：")  # 手动输入
    name = filename
    # file = ".\\result\\" + get_time_str() + "-" + name
    file = r'static/FracturePropagation/result/' + name
    mkdir(file)
    ...
    run = para_heading.add_run("分析报告_" + name + "_" + get_time_str())
    ...
    js_list = json.loads(data_json)
    # print(js_list)
    S1 = n = frac_num = frac_spac = Hw = Hp = Q = tp = E = v = ISIP = P_i = t_i = P_j = t_j = m = frac_type = 'undefined'
    for item in js_list:
        if item['name'] == 'S1':
            S1 = float(item['value'])
        elif item['name'] == 'n':
            n = float(item['value'])
        elif item['name'] == 'frac_num':
            frac_num = int(item['value'])
        elif item['name'] == 'frac_spac':
            frac_spac = float(item['value'])
        elif item['name'] == 'Hw':
            Hw = float(item['value'])
        elif item['name'] == 'Hp':
            Hp = float(item['value'])
            ...
            ISIP = float(item['value'])
        elif item['name'] == 'P_i':
            P_i = float(item['value'])
        elif item['name'] == 't_i':
            t_i = float(item['value'])
        elif item['name'] == 'P_j':
            P_j = float(item['value'])
        elif item['name'] == 't_j':
            t_j = float(item['value'])
        elif item['name'] == 'm':
            m = float(item['value'])
        elif item['name'] == 'frac_type':
            frac_type = str(item['value'])
    nl = 2 * n / (2 + n)  # 缝长比例系数
    ...
    paragraph.add_run().add_picture(
        ...
        file + "\\3D.png", width=Cm(12), height=Cm(8)
    )
    paragraph.add_run().add_picture(
        # file + "\\2D_" + time_str + ".png", width=Cm(12), height=Cm(8)
        file + "\\2D.png", width = Cm(12), height = Cm(8)
    )
...
# 从文件导入（名字.xsls）
def input_choose_y(filename, input_choose="y"):
    ...
    data_input = pd.ExcelFile(save_path_dir + name + ".xlsx")
